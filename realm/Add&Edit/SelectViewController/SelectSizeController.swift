...
final class SelectSizeViewController: UIViewController {
    ...
    let defaultSize = [Size(name: "Free")]
    var customSizes: Results<Size>!
    ...
    private func dataDidLoad() {
        customSizes = database.getAllDatas(Size.self, keyPath: "createdDate")
        selectedID = getSizeID(withName: item.size)
    }
    ...
    private func configureDataSource() {
        let cellRegistraion = UICollectionView.CellRegistration<UICollectionViewListCell, Size.ID> { [weak self] cell, indexPath, itemIdentifier in
            ...
            contentConfiguration.text = self.getSize(withID: itemIdentifier).name
    ...
    private func applySnapshot(reloading ids: [Size.ID] = []) {
        ...
        snapshot.appendItems(defaultSize.map { $0.id }, toSection: .defaultList)

        snapshot.appendItems(customSizes.map { $0.id }, toSection: .customList)
        if !ids.isEmpty {
...
extension SelectSizeViewController: UICollectionViewDelegate {
    ...
    private func updateSelectedId(collectionView: UICollectionView, indexPath: IndexPath) {
        ...
        newSnapshot.reconfigureItems(defaultSize.map { $0.id })
        newSnapshot.reconfigureItems(customSizes.map { $0.id })
        dataSource.apply(newSnapshot, animatingDifferences: false)
        
        print("selected:", getSize(withID: currentSelectedId).name)
        onchangeSize(getSize(withID: currentSelectedId).name)
    }
}
...
extension SelectSizeViewController: UITextFieldDelegate {
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        if validationText(with: text) {
            ...
            database.add(Size(name: modifiedText)) {[weak self] in
                self?.applySnapshot()
            }
        }
        textField.text = nil
...
extension SelectSizeViewController {
    ...
    private func getSizeID(withName name: String) -> Size.ID {
        guard let customSize = customSizes.first(where: { $0.name == name }) else {
            guard let defaultSize = defaultSize.first(where: { $0.name == name } ) else { fatalError("no have maching sizeName") }
            return defaultSize.id
        }
        return customSize.id
    }
    
    private func getSize(withID id: Size.ID) -> Size {
        guard let customIndex = customSizes.firstIndex(where: { $0.id == id }) else {
            guard let defaultIndex = defaultSize.firstIndex(where: { $0.id == id } ) else { fatalError("no have maching sizeID") }
            return defaultSize[defaultIndex]
        }
        return customSizes[customIndex]
    }
    
    private func deleteSize(withID id: Size.ID) {
        if selectedID == id {
            selectedID = getSizeID(withName: "Free")
            let selectedColor = getSize(withID: selectedID)
            onchangeSize(selectedColor.name)
        ...
        let size = customSizes.getSize(withID: id)
        print("deleted: \(size.name)")
        database.delete(size)
    }
}
